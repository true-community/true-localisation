...
for archive_dir in $(ls -1v ./content/${content_type}/); do
  ...
  echo "{ " > .dbtool/extracted/${content_type}/${archive_dir}/.index.json
  for direct_file in `echo $(cd .dbtool/extracted/${content_type}/${archive_dir}; find ./ -type f)`; do
    if [ -s ".dbtool/extracted/${content_type}/${archive_dir}/${direct_file}" ]; then
      echo "  \"${direct_file}\": \"${archive_dir}\"", >> .dbtool/extracted/${content_type}/${archive_dir}/.index.json
    else
      # remove blank files
      rm ".dbtool/extracted/${content_type}/${archive_dir}/${direct_file}" 2> /dev/null
    fi
  done
  truncate -s-2 .dbtool/extracted/${content_type}/${archive_dir}/.index.json
  echo "" >> .dbtool/extracted/${content_type}/${archive_dir}/.index.json
  echo "}" >> .dbtool/extracted/${content_type}/${archive_dir}/.index.json
  ...
  jq -s 'add' .dbtool/extracted/${content_type}/.index.json.tmp .dbtool/extracted/${content_type}/${archive_dir}/.index.json > .dbtool/extracted/${content_type}/.index.json
done
